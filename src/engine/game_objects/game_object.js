...
class GameObject {
    constructor(renderable) {
        ...
        this.mCollisionGroup = null // Collision Group of the GameObject *added by projectile project*
    }
    ...
    pixelTouches(otherObj, wcTouchPos) {
        ...
        // Checking if it can collide with its CollisionGroup *added by projectile project*
        if (!this.canCollideWithObj(otherObj)) { return false; }
    ...
    // Sets the CollisionGroup of the Projectile *added by projectile project*
    setCollisionGroup(cg) {
        this.mCollisionGroup = cg;
    }

    // Checks if the game_object can collide with this one in its CollisionGroup *added by projectile project*
    canCollideWithObj(otherObj) {
        if (this.mCollisionGroup === null) {
            if (otherObj.mCollisionGroup === null) {
                return true; // Assuming objects can collide if none belong to a CollisionGroup
            }
            return otherObj.mCollisionGroup.canCollide(null);
        }
        this.mCollisionGroup.canCollide(otherObj.mCollisionGroup);
    }
}
